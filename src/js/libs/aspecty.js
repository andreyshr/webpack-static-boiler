...
(function (root, factory) {
    ...
    aspecty.transform = rule => {
        ...
        ruleText.replace(/(--aspect-ratio:\s*)(\d\.*\d*\s*\/\s*\d\.*\d*)(\s*\!important)*\s*(?:;|\})/i, (string, property, value, important) => {
            ...
            selectorList.map(partial => {
                ...
                //console.log(partial,document.querySelectorAll(partial));
                Array.from(document.querySelectorAll(partial), (tag, i) => {

                    elWidth = tag.clientWidth || 0
                    ...
                    if (elWidth) {
                        ...
                        if (!tag.getAttribute('data-aspecty')) {
                            ...
                            if(img) {
                                img.addEventListener('load', aspecty.load)
                            }
    ...
    document.addEventListener("DOMContentLoaded", aspecty.load)

    aspecty.load();

    window.aspecty = aspecty;
